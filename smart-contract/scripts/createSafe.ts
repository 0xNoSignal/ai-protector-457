...
async function go() {
    ...
    let maxFeePerGas = ethers.BigNumber.from(80000000000) // fallback to 40 gwei
    let maxPriorityFeePerGas = ethers.BigNumber.from(80000000000)
    const tx = await aIProtector.create(
        ...
        initializer,
        saltNonce
        // { gasLimit: 1000000, nonce: 2, maxFeePerGas, maxPriorityFeePerGas }
    )
    console.log('tx', tx)
